                ...
                <div class="dashboard-content">
                    ...
                    <div class="check-in-out-section" style="margin-bottom: 2rem;">
                        <div class="check-in-out-card" style="background: #fff; border-radius: 12px; padding: 2rem; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                            <h2 style="margin-bottom: 1.5rem; color: #333;">Attendance</h2>
                            @if($todayLeave)
                                <div style="padding: 16px; background: #fff3cd; border: 1px solid #ffc107; border-radius: 8px; margin-bottom: 1rem;">
                                    <div style="display: flex; align-items: center; gap: 8px; color: #856404;">
                                        ...
                                        <div>
                                            ...
                                            <div style="font-size: 14px; margin-top: 4px;">
                                                {{ $todayLeave->leave_type_name ?? 'Leave' }} - 
                                                {{ date('M d', strtotime($todayLeave->start_date)) }} to {{ date('M d, Y', strtotime($todayLeave->end_date)) }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="check-in-out-buttons" style="display: flex; gap: 1rem; align-items: center;">
                                @if($todayLeave)
                                    <div style="padding: 12px 32px; font-size: 16px; border-radius: 8px; background: #e9ecef; color: #6c757d; cursor: not-allowed;">
                                        <i class="fas fa-ban"></i> CHECK IN (On Leave)
                                    </div>
                                @elseif(!$todayCheckIn)
                    ...
                    <!-- My Leaves Stats Section -->
                    <div class="leaves-stats-section" style="margin-bottom: 2rem;">
                        <div class="leaves-stats-card" style="background: #fff; border-radius: 12px; padding: 2rem; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
                                <h2 style="margin: 0; color: #333;">My Leaves Stats</h2>
                                <a href="{{ route('apply-leave') }}" class="btn btn-primary" style="padding: 10px 24px; border-radius: 6px; border: none; background: #007bff; color: white; text-decoration: none; cursor: pointer; display: inline-flex; align-items: center; gap: 8px;">
                                    <i class="fas fa-calendar-plus"></i> Apply Leave
                                </a>
                            </div>
                            <div class="leaves-stats-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1.5rem;">
                                <div class="stat-item" style="text-align: center; padding: 1.5rem; background: #f8f9fa; border-radius: 8px;">
                                    <div style="font-size: 32px; font-weight: bold; color: #007bff;">{{ $leavesStats['total'] }}</div>
                                    <div style="color: #666; margin-top: 0.5rem;">Total Leaves</div>
                                </div>
                                <div class="stat-item" style="text-align: center; padding: 1.5rem; background: #f8f9fa; border-radius: 8px;">
                                    <div style="font-size: 32px; font-weight: bold; color: #ffc107;">{{ $leavesStats['used'] }}</div>
                                    <div style="color: #666; margin-top: 0.5rem;">Used Leaves</div>
                                </div>
                                <div class="stat-item" style="text-align: center; padding: 1.5rem; background: #f8f9fa; border-radius: 8px;">
                                    <div style="font-size: 32px; font-weight: bold; color: #28a745;">{{ $leavesStats['remaining'] }}</div>
                                    <div style="color: #666; margin-top: 0.5rem;">Remaining Leaves</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="alerts-section">
                        <div class="alerts-card" style="background: #fff; border-radius: 12px; padding: 2rem; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                            <h2 style="margin-bottom: 1.5rem; color: #333;">My Alerts</h2>
                            <div class="alerts-list">
                                @forelse($alerts as $alert)
                                    <div class="alert-item" style="padding: 1rem; margin-bottom: 0.5rem; background: #f8f9fa; border-radius: 8px; border-left: 4px solid #007bff;">
                                        <div style="font-weight: 600; color: #333;">{{ $alert->data['title'] ?? 'Notification' }}</div>
                                        <div style="color: #666; margin-top: 0.25rem; font-size: 14px;">{{ $alert->data['message'] ?? '' }}</div>
                                        <div style="color: #999; margin-top: 0.5rem; font-size: 12px;">{{ $alert->created_at->diffForHumans() }}</div>
                                    </div>
                                @empty
